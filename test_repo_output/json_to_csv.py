...
def main():
    parser = argparse.ArgumentParser(
        description="Generate CSV from aider_repomap.json file"
    )
    parser.add_argument("json_file", help="Path to aider_repomap.json file")
    ...
    try:
        with open(args.csv_file, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for row in rows:
                writer.writerow(row)
        print(f"CSV file generated successfully at: {args.csv_file}")
    except Exception as e:
